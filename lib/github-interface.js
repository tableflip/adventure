...
async function getPullRequests ({ repos, version }) {
  const pullRequests = await request({
    methos: 'GET',
    url: `${githubApiBase}/repos/${repos[0]}/pulls?state=closed&base=master&sort=created`,
    auth,
    json: true,
    headers: githubHeaders
  })
  ...
  const tags = await request({
    method: 'GET',
    url: `${githubApiBase}/repos/${repos[0]}/tags`,
    auth,
    json: true,
    headers: githubHeaders
  })

  const tagShaDic = tags.reduce((dic, tag) => {
    const { name, commit: {sha} } = tag
    dic[sha] = name
    return dic
  }, {})

  const prData = pullRequests.map((pr) => {
    const { url, issue_url, commits_url, created_at, base: {sha}, number } = pr
    const version = tagShaDic[sha]
    return { number, url, issue_url, commits_url, created_at, master_sha: sha, version }
  })
