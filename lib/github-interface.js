...
//   tag: 'v2.0.0'
// })
function releaseStats ({ repos, tag }) {
  repos = arrayify(repos)
  const stats = repos.map((repo) => repoReleaseStats({ repo, tag }))
  return Promise.all(stats)
    .then((reposData) => {
      return reposData.reduce((memo, repoData) => {
        return {
          commits: memo.commits + repoData.commits,
          files: memo.files + repoData.files,
...
// Gets the aggregate stats for the given repo and tag, by comparing it to the previous tag if available,
// or summarising the whole history if not
async function repoReleaseStats ({ repo, tag }) {
  const releaseTags = await getTags({ repo })
  const tagIndex = releaseTags.indexOf(tag)

  if (tagIndex === -1) throw new Error(`There is no release tagged ${tag}`)

  const previousTag = releaseTags[tagIndex + 1]

  if (previousTag) {
    return getCommitComparison({ repo, lastCommit: tag, firstCommit: previousTag })
  } else {
...
async function getTesterFeedback ({
  ...
  debug(`getting tester feedback for tag ${tag} of ${repo}`)
  ...
  const releaseIndex = releases.findIndex((r) => r.name === tag)
  ...
  if (releaseIndex > 0) {
    const majorVersionNumber = semver.major(tag, true)
  ...
  debug(`requesting issues from ${tag} to ${nextMajorRelease ? nextMajorRelease.name : 'now'}`)
